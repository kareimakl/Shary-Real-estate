...
function deliveryFilter(type) {
  ...
  if (type === "open") {
    ...
    deliveryFilter.classList.remove("hidden");

    setTimeout(() => {
      deliveryFilter.style.maxHeight = "50dvh";
      deliveryFilter.style.transition = "max-height 0.3s ease-in-out";
    }, 10);

    document.body.style.overflow = "hidden";
  } else {
    deliveryFilter.style.maxHeight = "0px";
    deliveryFilter.style.transition = "max-height 0.3s ease-in-out";
    document.body.style.overflow = "auto";

    // أخفي العنصر والـ overlay بعد الأنيميشن
    setTimeout(() => {
      deliveryFilter.classList.add("hidden");
      overlay.classList.add("hidden");
    }, 300);
  }
}
function propertyFilter(type) {
  const propertyFilter = document.getElementById("propertyfilter");
  const overlaytow = document.getElementById("overlaytow");

  if (type === "open") {
    overlaytow.classList.remove("hidden");
    propertyFilter.classList.remove("hidden");

    setTimeout(() => {
      propertyFilter.style.maxHeight = "50dvh";
      propertyFilter.style.transition = "max-height 0.3s ease-in-out";
    }, 10);

    document.body.style.overflow = "hidden";
  } else {
    propertyFilter.style.maxHeight = "0px";
    propertyFilter.style.transition = "max-height 0.3s ease-in-out";
    document.body.style.overflow = "auto";

    // أخفي العنصر والـ overlay بعد الأنيميشن
    setTimeout(() => {
      propertyFilter.classList.add("hidden");
      overlaytow.classList.add("hidden");
    }, 300);
  }
}
function priceFilter(type) {
  ...
  if (type === "open") {
    ...
    setTimeout(() => {
      priceFilter.style.maxHeight = "50dvh";
      priceFilter.style.transition = "max-height 0.3s ease-in-out";
    }, 10);

    document.body.style.overflow = "hidden";
  } else {
    priceFilter.style.maxHeight = "0px";
    priceFilter.style.transition = "max-height 0.3s ease-in-out";
    document.body.style.overflow = "auto";
...
document.addEventListener("click", (event) => {
  ...
  for (let i = 0; i < dropdownBtns.length; i++) {
    ...
    if (
      ...
      !event.target.classList.contains("dropdown-btn-footer")
    ) {
    ...
    if (
      ...
      allDropdowns[i].contains(event.target) &&
      element.classList.contains("invisible", "opacity-0")
    ) {
      if (
        event.target.tagName === "IMG" ||
        event.target.classList.contains("img-container")
      ) {
        dropdownBtns[i].querySelector("img").src =
          event.target.src || event.target.querySelector("img").src;
      } else {
...
function toggleMoreLessText(elementName) {
  ...
  btnText.innerHTML = content.classList.contains("hidden")
    ? "Show More"
    : "Show Less";
  btnIcon.innerHTML = content.classList.contains("hidden")
    ? "keyboard_arrow_down"
    : "keyboard_arrow_up";
}
...
function openSideSection(elementId, overlayId) {
  const targetElement = document.getElementById(elementId);
  const sideFilterOverlay = document.getElementById(
    overlayId ? overlayId : "sideFilterOverlay"
  );
  targetElement.classList.toggle("invisible");
  targetElement.classList.toggle("opacity-0");
...
function updateRangeSlider(containerName) {
  ...
  const beforeElement =
    rangeSlider.querySelector(".range-slider-before") ||
  ...
  Object.assign(beforeElement.style, {
    ...
    backgroundImage: `linear-gradient(to ${
      localStorage.getItem("language") === "en" ? "right" : "left"
    }, transparent ${lowPercent}%, var(--range-slider-active-color) ${lowPercent}%, var(--range-slider-active-color) ${highPercent}%, transparent ${highPercent}%)`,
    pointerEvents: "none",
  });
  if (!rangeSlider.contains(beforeElement))
    rangeSlider.appendChild(beforeElement);
  const currentResult = document.querySelector(
    `.current-${containerName}-range`
  );
  const unit = currentResult.getAttribute("data-unit") || "EGP";
  if (currentResult)
    currentResult.textContent = `${lowValue.toLocaleString()} ${unit} - ${highValue.toLocaleString()} ${unit}`;
}
